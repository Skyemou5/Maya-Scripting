...
class MainUI():
    ...
    def create(self):
        ...
        ### random tool ###
        cmds.button(parent=self.col_layout, label='Rename Selection',
                    c=lambda *x: rename())
        ### random tool label ###
        cmds.separator(h=10, style='none')
        ...
        ### random input fields ###
        ### side by side inputs ### a
        # cmds.rowColumnLayout(parent=self.my_window,
        #                      adjustableColumn=True,
        #                      numberOfColumns=3,
        #                      columnWidth={(90, 80),(90, 80),(90, 80)})

        #cmds.setParent('..')

        cmds.button(parent=self.col_layout, label='Scatter Objects',
                    c=lambda *x: scatter(self.random_trans_field,self.random_rot_field,self.random_rot_field))


        #cmds.rowColumnLayout(parent=self.my_window,adjustableColumn=True,width=50)
        cmds.text("Random_Translation")
        self.random_trans_field = cmds.floatField(value = 5)

        #cmds.rowColumnLayout(parent=self.my_window,adjustableColumn=True)
        cmds.text("Random_Rotation")
        self.random_rot_field = cmds.floatField(value = 5)

        #cmds.rowColumnLayout(parent=self.my_window,adjustableColumn=True)
        cmds.text("Random_Scale")
        self.random_scale_field = cmds.floatField(value = 5)
